...
//!
//! **Platform support:** Linux (full), macOS (utun), Windows (stub — use Wintun adapter).
...
#[cfg(not(target_os = "windows"))]
pub use unix_impl::TunInterface;

/// Windows stub: TUN/TAP requires a Wintun adapter installed on the system.
/// The mesh-overlay binary does not run on Windows in this version.
/// Use mesh-tunnel (client mode) for Windows endpoints.
...
pub mod windows_stub {
    ...
    impl TunInterface {
        pub fn new(_address: &str, _netmask: &str, _mtu: u16) -> io::Result<Self> {
            Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "TUN interface is not supported on Windows. Use mesh-tunnel (client mode) instead, \
                 or deploy mesh-overlay on a Linux/macOS gateway.",
            ))
        }
        ...
        pub async fn read_packet(&self) -> io::Result<Vec<u8>> {
            Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "TUN read not supported on Windows",
            ))
        }

        pub async fn write_packet(&self, _packet: &[u8]) -> io::Result<usize> {
            Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "TUN write not supported on Windows",
            ))
        }
    }
}
...
#[cfg(target_os = "windows")]
pub use windows_stub::TunInterface;
