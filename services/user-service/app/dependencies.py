"""
FastAPI dependencies for user-service: JWT auth and internal service verification.
"""
...
async def verify_internal_service(request: Request):
    """
    Verify internal service-to-service calls by checking the X-Internal-API-Key header.
    Used by the API Gateway to pass authenticated context to downstream services.
    """
    ...
    if not _constant_time_compare(api_key, settings.INTERNAL_API_KEY):
        raise HTTPException(
