"""
Shared middleware: rate limiting (Redis sliding window) and request logging.
Mirrors the original monolith's middleware with fail-closed behavior.
"""
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    globally across all API replicas. Falls back to in-memory with a
    conservative budget when Redis is unavailable (fail-closed).
    """
...
class LoggingMiddleware(BaseHTTPMiddleware):
    """Logs every HTTP request with timing and status in JSON format."""
